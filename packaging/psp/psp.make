...
# -nostartfiles, the flag pspsdkcc() already picks for SHARED_LDFLAGS and for
# the same reason it documents there: it suppresses psp-gcc's own crt0 (which
# references main()) while keeping automatic libc/libgcc linking. Without it
# the link drags in SDK stub libraries whose .lib.stub sections want
# __executable_start, a symbol the default linker script defines and
# plugin.link does not.
PLUGINLDFLAGS = -nostartfiles -T$(PLUGINLINK_LDS) -Wl,--gc-sections \
